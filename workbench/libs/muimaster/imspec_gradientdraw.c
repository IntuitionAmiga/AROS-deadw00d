...
//    double vy = sinarc;
    ...
    startColour.red = start_rgb[0];
    startColour.green = start_rgb[1];
    startColour.blue = start_rgb[2];

    endColour.red = end_rgb[0];
    endColour.green = end_rgb[1];
    endColour.blue = end_rgb[2];

    diffR = endColour.red - startColour.red;
    diffG = endColour.green - startColour.green;
    diffB = endColour.blue - startColour.blue;
    ...
    if (angle > 90 && angle <= 270)
    {
         ...
         * some parameters.
         *
         * Note that if one would exchanging startColour/endColour the values would
    ...
    for (l = 0, y = ystart + ((yp - yt) * yadd); l < h; l++, y += yadd)
    {
        ...
        for (c = 0, x = ((xp - xt) * xadd); c < w; c++, x += xadd)
        {
            ...
            red = startColour.red + (int)(diffR * y1 / height);
            green = startColour.green + (int)(diffG * y1 / height);
            blue = startColour.blue + (int)(diffB * y1 / height);
...
//    double vy = sinarc;
    ...
    if (angle > 90 && angle <= 270)
    {
         ...
         *
         * Note that if one would exchanging startColour/endColour the values would only work
