/*
    ...
    Desc: BeginIO - Start up a timer.device request.
          Adapted from arch/m68k-amiga/timer/beginio.c.
          Logic is identical; only timer_intern.h differs.
*/
...
static void timer_addToWaitList(struct TimerBase *TimerBase, UWORD unit, struct timerequest *iotr)
{
    ...
    D(bug("added %x: %d/%d->%d/%d\n", iotr,
        ...
        (unit == UNIT_VBLANK ? TimerBase->tb_vb_count.tv_micro : TimerBase->tb_micro_count.tv_micro),
