/*
    ...
    Desc: IE audio.device — main device logic.
          Identical to the m68k-amiga audio.device; only the hardware
          layer (audio_hardware.c) differs.  All Paula channel semantics
          (allocation, write queues, cycles, PERVOL, FINISH, etc.) are
...
static int GM_UNIQUENAME(init)(LIBBASETYPEPTR AudioBase)
{
    ...
    AudioBase->zerosample = AllocMem(2, MEMF_PUBLIC | MEMF_CLEAR);
